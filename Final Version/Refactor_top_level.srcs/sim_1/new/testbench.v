...
module testbench;
    ...
    reg [`WIDTH_PORT-1:0] x_in_tb; // Input to the DUT
    wire [`WIDTH_PORT-1:0] y_out_tb;   // Output from the DUT
    wire [`WIDTH_TIME-1:0] done_out_tb; // Done signal from the DUT
    
    // Instantiate the top-level module (Design Under Test)
    top_level dut (
        .clk(clk),
        .rst(rst),
        .x_in_tb(x_in_tb),
